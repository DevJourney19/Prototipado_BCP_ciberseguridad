...
var_dump("La dirección ip es: ".$dir_ip);
...
try {
    ...
    if (count($registro) === 1) {
        ...
        if ($registro2[0]['activacion_seguridad'] === 1) { //Va a revisar si esta activado la seguridad
            ...
            $id_seguridad = (int) $id_seguridad;
            var_dump("El id de seguridad es: ".$id_seguridad);
            //DISPOSITIVO
            $registro3 = $daoDispositivo->enterAccess($id_seguridad, $dir_ip);
            var_dump("El registro3 es: ". $registro3);
            var_dump($registro3);
            echo "=================";
            
            $direccion_ip_deseada = $registro3[0]['direccion_ip'];    
            var_dump("La direccion deseada es: ".$direccion_ip_deseada);        
            echo "------------------------------";
            $_SESSION['info'] = $info;
            ...
            if (!empty($direccion_ip_deseada) && $_SESSION['direccion_ip'] === $direccion_ip_deseada) {
                ...
                if ($horario) {
                    ...
                    // si está fuera del rango de restriccion de hora
